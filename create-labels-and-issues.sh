#!/bin/bash
REPO="Dot-Voidz/Stellar-Wallet-Dashboard"

# First, create all required labels
echo "Creating labels..."
gh label create --repo "$REPO" "Stellar Wave" --color "238636" --description "Issues eligible for Drips Wave" 2>/dev/null || true
gh label create --repo "$REPO" "trivial" --color "7057ff" --description "100 points - Simple task" 2>/dev/null || true
gh label create --repo "$REPO" "medium" --color "a371f7" --description "150 points - Moderate task" 2>/dev/null || true
gh label create --repo "$REPO" "high" --color "d73a4a" --description "200 points - Complex task" 2>/dev/null || true
gh label create --repo "$REPO" "bug" --color "d73a4a" --description "Something isn't working" 2>/dev/null || true
gh label create --repo "$REPO" "enhancement" --color "a2eeef" --description "New feature or request" 2>/dev/null || true
gh label create --repo "$REPO" "documentation" --color "0075ca" --description "Improvements or additions to documentation" 2>/dev/null || true
gh label create --repo "$REPO" "testing" --color "cfd3d7" --description "Improvements or additions to tests" 2>/dev/null || true
gh label create --repo "$REPO" "ui/ux" --color "c5def5" --description "Improvements or additions to UI/UX" 2>/dev/null || true
...
echo "✅ All labels and 20 issues created!"
